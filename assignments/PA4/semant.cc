...
ClassTable::ClassTable(Classes classes) : semant_errors(0) , error_stream(cerr) {
	...
	for (int i = classes->first(); classes->more(i); i = classes->next(i)){
		...
		if (curname == SELF_TYPE || curname == Int || curname == IO || curname == Bool || curname == Str || curname == Object) {
			semant_error(curclass) << "Error! Redefinition of basic class " << curname << std::endl;
			return;
		}
		
		if (mp_class.find(curname) != mp_class.end()) {
			semant_error(curclass) << "Error! Class " << curname << " was previously defined" << std::endl;
			return;
		}
...
void ClassTable::DFS(Class_ cur, Classes classes, std::map<Class_,int> &vis,bool &err, bool &cycle) {
	
	log << "DFS class " << cur->get_name() << std::endl;
	...
	if (mp_class.find(parent_name) == mp_class.end()) {
		semant_error(cur) << "Error! Class " << cur->get_name() <<" inherits from an undefined class " << parent_name << std::endl;
		err = true;
		return;
	}

	if (parent_name == Int || parent_name == Str || parent_name == Bool || parent_name == SELF_TYPE) {
		semant_error(cur) << "Error! Class " << cur->get_name() << " cannot inherit " << parent_name << std::endl;
		err = true;
		return;
	}
	...
	if (cycle)
		semant_error(cur) << "Error! Class " << cur->get_name() << " is involved in an inheritance cycle" << std::endl;
	if (err)
		return;
	vis[cur] = 2;
}
...
Symbol method_class::matchFormals(Expressions actual) {
  Symbol type = return_type;
  for (int i = actual->first(); actual->more(i); i = actual->next(i)) {
	if ((formals->more(i+1) && !actual->more(i+1)) || (!formals->more(i+1) && actual->more(i+1))) {
		classtable->semant_error(curclass) << "Method " << name <<" called with wrong number of arguments." << endl; 
    	return Object;
    }
